...
def info():
    console = Console()
    console.print("\n\n"
                  "\t\t The " + colorstr('red bold', 'Rainbow DQN') + " software operates as a RL framework \n"
                  "\t\t for efficient discrete action space exploration. \n"
                  "\n")
...
def get_options():
    ...
    >>> python train.py --device 'cuda:0' --mem-capacity 100000 --batch-size 128 --target-sync 1000 --learning-starts 100000 --num-of-steps-to-checkpoint-model 1000 --num-of-steps-to-checkpoint-memory 10000 --episodes 10000 --gamma 0.99 --alpha 0.4 --beta 0.6 --prior-eps 1e-6 --v-min -21.0 --v-max 20.0 --n-atoms 51 --n-step 3 --verbosity 10 --learning-rate 2.5e-4 --info --use-deterministic-algorithms --clip-grad-norm 40.0 --top-k 10 --num-hiddens 512 --wandb --tensorboard --name "Rainbow" --store "options" --verbose --seed 33
    """
    ...
    parser.add_argument('--en-cnn', action="store_true", 
                        help='utilize grayscale observations instead of vectors')
...
def demo_args():
    ...
    >>> python test.py --device 'cuda:0' --episodes 10000 --verbosity 10 --info --use-deterministic-algorithms --name "Rainbow" --sync "data" --verbose --seed 33
    """
    parser = argparse.ArgumentParser(
        description=f'Parser for the Rainbow DQN.\n'
                    f'For information on the Rainbow DQN software, pass `--info`')
...
def store_args(filename, args):
    ...
    args : argparse.ArgumentParser
        Parser for the Rainbow framework.
    filename : str
