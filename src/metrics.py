...
class MetricLogger:
    ...
    def log_env_info(self):
        ...
        if wandb.run:
            wandb.log({"Environment Info": wandb.Html(
                env_details.replace("\n", "<br>"), inject=False)})
    ...
    def record(self, episode, step, verbose=False):
        ...
        if self.tensorboard_writer:
            self.tensorboard_writer.add_scalar(
                'Episode/Min Return', np.round(np.min(self.ep_rewards[-self.verbosity:]), 3), episode)
            self.tensorboard_writer.add_scalar(
                'Episode/Max Return', np.round(np.max(self.ep_rewards[-self.verbosity:]), 3), episode)
            self.tensorboard_writer.add_scalar(
                'Episode/Mean Reward', mean_ep_reward, episode)
            self.tensorboard_writer.add_scalar(
                'Episode/Mean Length', mean_ep_length, episode)
            self.tensorboard_writer.add_scalar(
                'Episode/Mean Loss', mean_ep_loss, episode)
            self.tensorboard_writer.add_scalar(
                'Episode/Time Delta', time_since_last_record, episode)
