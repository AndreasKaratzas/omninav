...
def test_stats(towards_target: list = [], mission_status: list = []):
    ...
    y_1 = np.array([sum(towards_target), len(towards_target) -
                   sum(towards_target)], dtype=np.int32)
    y_2 = np.array([sum(mission_status), len(mission_status) -
                   sum(mission_status)], dtype=np.int32)
